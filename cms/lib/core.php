...
{
    return cms_write_file($file, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE), $mode);
}
...
function cms_yaml_value($v)
{
    ...
    return json_encode($v, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);
}
...
function cms_types()
{
    return array(
        ...
        'talks' => array('label' => 'Talks & media', 'singular' => 'talk'),
    );
}
...
function cms_normalise_item($type, $slug, array $meta, $body, $mtime)
{
    ...
    $item = array(
        ...
        // Talks
        'kind' => isset($meta['kind']) ? (string) $meta['kind'] : 'talk',
        'event' => isset($meta['event']) ? (string) $meta['event'] : '',
        'location' => isset($meta['location']) ? (string) $meta['location'] : '',
        'post' => isset($meta['post']) ? (string) $meta['post'] : '',
        // Projects: interactive 3D model (.glb) shown on the project page / homepage
        'model' => isset($meta['model']) ? (string) $meta['model'] : '',
        'model_home' => !empty($meta['model_home']),
        // Share preview image generated by the admin
        'og_image' => isset($meta['og_image']) ? (string) $meta['og_image'] : '',
        'body' => $body,
        'updated' => $mtime,
        'meta' => $meta,
    );
    return cms_localize($item);
}
...
function cms_list_items($type, $withDrafts = false)
{
    ...
    foreach (glob($dir . '/*.md') as $file) {
        ...
        // Scheduled posts/news stay hidden until their date; upcoming talks are shown.
        if (!$withDrafts && ($type === 'posts' || $type === 'news') && $it['date'] > time()) continue;
        $items[] = $it;
    }
...
function cms_talk_url($slug)
{
    return cms_url('talks/#' . rawurlencode($slug));
}
