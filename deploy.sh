...
mkdir -p "\$DATA"/posts "\$DATA"/news "\$DATA"/projects "\$DATA"/publications "\$DATA"/talks "\$DATA"/history "\$DATA"/trash "\$DATA"/sessions "\$DATA"/state "\$WEB/files/uploads"

# Import existing posts, news and projects without overwriting anything already there.
tmp=\$(mktemp -d)
tar -xzf - -C "\$tmp"
for kind in posts news projects publications talks; do
  for f in "\$tmp/\$kind"/*.md; do
    [ -e "\$f" ] || continue
    [ -e "\$DATA/\$kind/\$(basename "\$f")" ] || cp "\$f" "\$DATA/\$kind/"
  done
done
for doc in cv.json repositories.json home.json; do
